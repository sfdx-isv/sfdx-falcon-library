 ...
 * @file          packages/util/src/jsforce.ts
...
import  {JsonCollection}            from  '@sfdx-falcon/types';     // Type. Any valid JSON collection value (JsonMap or JsonArray).
...
const dbgNs = '@sfdx-falcon:util:jsforce';
...
export async function changePassword(aliasOrConnection:AliasOrConnection, userId:string, newPassword:string):Promise<void> {
  ...
  const dbgNsLocal = `${dbgNs}:changePassword`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);

  // Validate arguments
  TypeValidator.throwOnEmptyNullInvalidString(userId,       `${dbgNsLocal}`, `userId`);
  TypeValidator.throwOnEmptyNullInvalidString(newPassword,  `${dbgNsLocal}`, `newPassword`);
...
export async function createSfdxOrgConfig(aliasOrConnection:AliasOrConnection, username:string, password:string, orgAlias:string):Promise<void> {
  ...
  // Define local debug namespace and debug incoming arguments.
  const dbgNsLocal = `${dbgNs}:createSfdxOrgConfig`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);
...
export async function describeSignupRequest(aliasOrConnection:AliasOrConnection):Promise<ObjectDescribe> {
  ...
  // Define local debug namespace and debug incoming arguments.
  const dbgNsLocal = `${dbgNs}:describeSignupRequest`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);
  ...
  const describeResponse = await restApiRequest(describeRequest)
    .catch(error => {
      ...
      SfdxFalconDebug.obj(`${dbgNsLocal}:error:`, error);
      return Promise.resolve({});
    }) as JsonMap;
  
  // DEBUG
  SfdxFalconDebug.obj(`${dbgNsLocal}:describeResponse:`, describeResponse);
...
export async function getAssignedPermsets(aliasOrConnection:string|Connection, userId:string):Promise<string[]> {
  ...
  // Define local debug namespace and debug incoming arguments.
  const dbgNsLocal = `${dbgNs}:getAssignedPermsets`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);
  ...
  SfdxFalconDebug.obj(`${dbgNsLocal}:queryResult.records:`, queryResult.records);
  ...
  SfdxFalconDebug.obj(`${dbgNsLocal}:assignedPermsets:`, assignedPermsets);
 ...
 *              `QueryResult` containing the `MetadataPackage` objects and their child objects.
 *              This has the effect of detailing all of the packages (managed and unmanaged) that
 *              are developed in that org.
 * @public @async
 */
// ────────────────────────────────────────────────────────────────────────────────────────────────┘
export async function getPackages(aliasOrConnection:AliasOrConnection):Promise<QueryResult<MetadataPackage>> {
  ...
  // Define local debug namespace and debug incoming arguments.
  const dbgNsLocal = `${dbgNs}:getPackages`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);
  ...
  SfdxFalconDebug.obj(`${dbgNsLocal}:metadataPackageResults:`, metadataPackageResults);
...
export async function getProfileId(aliasOrConnection:AliasOrConnection, profileName:string):Promise<string> {
  ...
  // Define local debug namespace and debug incoming arguments.
  const dbgNsLocal = `${dbgNs}:getProfileId`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);
  ...
  SfdxFalconDebug.obj(`${dbgNsLocal}:queryResult.records[0]`, queryResult.records[0]);

  // Make sure we got a result.  If not, throw error.
  if (typeof queryResult.records[0] === 'undefined') {
    throw new SfdxFalconError ( `Profile '${profileName}' does not exist in org '${rc.orgIdentifier}'`
                              , `ProfileNotFound`
                              , `${dbgNsLocal}`);
  }
...
export async function getUserId(aliasOrConnection:AliasOrConnection, username:string):Promise<string> {
  ...
  // Define local debug namespace and debug incoming arguments.
  const dbgNsLocal = `${dbgNs}:getUserId`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);
  ...
  // Make sure we got a result.  If not, throw error.
  if (typeof queryResult.records[0] === 'undefined') {
    throw new SfdxFalconError ( `User '${username}' does not exist in org '${rc.orgIdentifier}'`
                              , `UserNotFound`
                              , `${dbgNsLocal}`);
  }
...
export async function restApiRequest(restRequestDef:RestApiRequestDefinition):Promise<JsonCollection> {
  ...
  // Define local debug namespace and debug incoming arguments.
  const dbgNsLocal = `${dbgNs}:restApiRequest`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);

  // Validate incoming arguments.
  TypeValidator.throwOnEmptyNullInvalidObject(restRequestDef,  `${dbgNsLocal}`, `restRequestDef`);
  ...
  SfdxFalconDebug.obj(`${dbgNsLocal}:restResult:`, restResult);
...
export async function restApiRequestRaw(restRequestDef:RestApiRequestDefinition):Promise<RawRestResponse> {
  ...
  // Define local debug namespace and debug incoming arguments.
  const dbgNsLocal = `${dbgNs}:restApiRequestRaw`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);
  ...
  SfdxFalconDebug.obj(`${dbgNsLocal}:rawResult:`, rawResult);
...
export async function toolingApiQuery<T>(aliasOrConnection:AliasOrConnection, query:string):Promise<QueryResult<T>> {
  ...
  // Define local debug namespace and debug incoming arguments.
  const dbgNsLocal = `${dbgNs}:toolingApiQuery`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);
  ...
  return new Promise((resolve, reject) => {
    rc.connection.tooling.query<T>(query, {}, (err, records) => {
      if (err) {
        SfdxFalconDebug.obj(`${dbgNsLocal}:err:`,     err,      `Callback returned with the following Error: `);
        SfdxFalconDebug.obj(`${dbgNsLocal}:records:`, records,  `Records returned as part of Query Error: `);
        reject(err);
      }
      SfdxFalconDebug.obj(`${dbgNsLocal}:records:`, records,  `Records successfully returned: `);
      resolve(records);
    });
  });
}
